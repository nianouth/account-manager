...
1. 读取 `manifest.json` 获取当前版本号，将 patch 版本号 +1（如 `1.2.3` → `1.2.4`），更新 `manifest.json` 中的 `version` 字段
1.1. 同步更新 `popup.html` 设置页面中的版本号显示（搜索 `<span class="setting-value">v` 并替换为新版本号）
2. 读取当前 `CHANGELOG.md`
3. 使用**新版本号**，在 CHANGELOG 顶部（`---` 分隔线后）追加新条目或更新当天已有条目
4. 使用项目已有的 CHANGELOG 格式（Keep a Changelog 风格，带 emoji 分类）
5. 如果当天已有相同版本号的条目，在该条目中追加新的变更项，而不是创建重复条目
6. 将 `manifest.json` 和 `CHANGELOG.md` 的变更加入暂存区
