...
一个功能强大的 Chrome 浏览器扩展，专为多环境、多账号管理而设计
...
全房通账号管理器是一个专业的 Chrome 浏览器扩展插件，旨在帮助用户高效管理多个环境（开发、测试、生产等）下的多个账号。通过智能环境匹配、一键自动登录、数据加密存储等核心功能，让账号管理变得简单、安全、高效。

### 主要特点

- 🎯 **智能环境匹配**：自动识别当前页面所属环境，无需手动切换
...
- 在环境列表中点击 **编辑** 按钮修改环境信息
- 点击 **删除** 按钮删除环境
- ⚠️ **注意**：删除环境会同时删除该环境下的所有账号
...
**A:** 在环境配置中填写登录按钮的 ID 或 Class。如果不填写，系统会使用默认值（ID: `ch_login_btn`, Class: `formBtn`）。如果默认值不匹配，可以：
1. 打开目标网站的登录页面
